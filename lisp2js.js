...
LISP['register-macro'](LISP.intern("labels"), (function(lss){var body = LISP._getRestArgs(arguments, 1); return (LISP["list*"](LISP.intern("let"), LISP.map((function(ls){return (LISP.car(ls));}), lss), LISP.append(LISP.map((function(ls){return (LISP.list(LISP.intern("set!"), LISP.car(ls), LISP["list*"](LISP.intern("^"), LISP.cdr(ls))));}), lss), body)));}));
...
LISP["local-var?"] = (function(sym, scope){return ((LISP.isTrue(LISP["symbol?"](sym)) ? (LISP["scope-var?"](scope, LISP["get-receiver"](sym))) : (LISP.nil)));});
